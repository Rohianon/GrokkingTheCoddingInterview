...
public class NoRepeatSubstring {
    public static int findLength(String str) {
        ...
        for (int windowEnd = 0; windowEnd < str.length(); windowEnd++) {
            ...
            // if the map already contains 'rightChar', shrink the window from the
            // beginning so that we have  only one occurrence of 'rightChar'
            if (charIndexMap.containsKey(rightChar)) {
                // this is tricky; in the current window, we will not have 'rightChar' after
                // its previous index and if 'windowStart' is already a head o the last index of the
